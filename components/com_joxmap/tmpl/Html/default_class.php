...
use JLTRY\Component\JoXmap\Site\Controller\JoXmapDisplayer;
...
class JoXmapHtmlDisplayer extends JoXmapDisplayer {
    ...
    function __construct ($config, $sitemap) {
        ...
        $this->live_site = substr_replace(Uri::root(), "", -1, 1);
    ...
    */
    function printNode( &$node )
    {
        ...
        if ($node->browserNav != 3) {
            $link = Route::_($node->link, true, @$node->secure);
        }
        ...
        switch( $node->browserNav ) {
            case 1:        // open url in new window
                ...
                if ( $this->sitemap->params->get('exlinks') ) {
                    $ext_image = '&nbsp;<img src="'. $this->live_site .'/components/com_xmap/assets/images/'. $this->sitemap->params->get('exlinks') .'" alt="' . JText::_('COM_JOXMAP_SHOW_AS_EXTERN_ALT') . '" title="' . JText::_('COM_JOXMAP_SHOW_AS_EXTERN_ALT') . '" border="0" />';
                }
